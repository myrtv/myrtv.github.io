//Seems they finally reached something consistent: 360p, 720p, 1080p, etc.
...
$.each(archive_input, function (index, val) {
  ...
  if (index > 0 && temp[temp.length - 1].name == item.name) {
    ...
    //Sort qualities.
    var order = ['360p', '720p', '1080p'];
    temp[temp.length - 1].qualities.sort(function (a, b) {
      return (order.indexOf(a.bitrate) == -1 && order.indexOf(b.bitrate) == -1) ? 0 :
             (order.indexOf(a.bitrate) == -1) ? 1 : 
             (order.indexOf(b.bitrate) == -1) ? 1 : 
             (order.indexOf(a.bitrate) < order.indexOf(b.bitrate)) ? -1 : 1;
    });
  } else {
    //Unique entry
    temp.push(item);
...
function itemParse(val) {
  return {
    name: val.title.match(/^\d+\. (?:Awesome|Summer)GamesDoneQuick\d+ (?:part\d+ )?(.*?) \d+p$/i)[1],
    duration: Math.round(val.duration),
    qualities: [
      {
        label: val.sources[0].label,
        thumbnail: val.image,
        bitrate: val.title.match(/(\d+p)$/i) [0],
        size: 0,
